...
public extension DerivationApiClient {
...
//    }
    ...
    ) async throws -> DerivableSPLTokenDestinationAddress {
        var address: String
...
//            } else if accountInfo?.owner == SystemProgram.id.base58EncodedString {
            } else if accountInfo?.owner == SafeCoinTokenProgram.systemProgramId.base58EncodedString {
                let owner = try PublicKey(string: destinationAddress)
                let tokenMint = try PublicKey(string: mintAddress)
                // create associated token address //TODO programs ids
...
//            if info?.owner == TokenProgram.id.base58EncodedString, info?.data != nil {
          if info?.owner == SafeCoinTokenProgram.tokenProgramId.base58EncodedString, info?.data != nil {
