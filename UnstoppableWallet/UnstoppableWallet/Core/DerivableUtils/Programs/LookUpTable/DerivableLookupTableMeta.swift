...
struct DerivableLookupTableMeta: BufferLayout {
...
public struct DerivableAddressLookupTableState: Equatable, Codable, BufferLayout {
    let typeIndex: UInt32
    let deactivationSlot: UInt64
    let lastExtendedSlot: UInt64
    ...
    public init(from reader: inout BinaryReader) throws {
        ...
        try readerForAddresses.read(count: DerivableAddressLookupTableAccount.lookUpTableMetaSize)
...
public struct DerivableAddressLookupTableAccount: Equatable {
    public static let lookUpTableMetaSize = 56

    public let key: PublicKey
    public let state: DerivableAddressLookupTableState

    public init(key: PublicKey, state: DerivableAddressLookupTableState) {
        self.key = key
        self.state = state
    }
