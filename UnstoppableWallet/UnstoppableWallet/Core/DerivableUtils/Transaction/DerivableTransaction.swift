...
public struct DerivableTransaction: Encodable, Equatable {
    public var signatures = [DerivableSignature]()
...
//        TODO: nonceInfo
    ...
    public mutating func sign(signers: [DerivableKeyPair]) throws {
        guard !signers.isEmpty else {
            throw DerivableVersionedTransactionError.noSigner
        }

        // unique signers
        let signers = signers.reduce([DerivableKeyPair]()) { signers, signer in
        ...
        // map signatures
        signatures = signers.map { DerivableSignature(signature: nil, publicKey: $0.publicKey) }
    ...
    public mutating func serialize(
        ...
        if verifySignatures, !_verifySignatures(
            ...
            throw DerivableVersionedTransactionError.signatureVerificationError
        }
    ...
    public mutating func addSignature(_ signature: DerivableSignature) throws {
    ...
    public func findSignature(pubkey: PublicKey) -> DerivableSignature? {
        signatures.first(where: { $0.publicKey == pubkey })
    }

    // MARK: - Signing

    public mutating func partialSign(signers: [DerivableKeyPair]) throws {
        guard !signers.isEmpty else {
            throw DerivableVersionedTransactionError.noSigner
        }
    ...
    private mutating func partialSign(message: DerivableMessage, signers: [DerivableKeyPair]) throws {
        let signData = try message.serialize()

        for signer in signers {
            let data = try NaclSign.signDetached(message: signData, secretKey: signer.secretKey)
            try _addSignature(DerivableSignature(signature: data, publicKey: signer.publicKey))
        }
    }

    private mutating func _addSignature(_ signature: DerivableSignature) throws {
        guard let data = signature.signature,
              data.count == 64,
              let index = signatures.firstIndex(where: { $0.publicKey == signature.publicKey })
        else {
            throw DerivableVersionedTransactionError.invalidSigner(signature.publicKey.base58EncodedString)
        }
    ...
    private mutating func compile() throws -> DerivableMessage {
        ...
        signatures = signedKeys.map { DerivableSignature(signature: nil, publicKey: $0) }
        return message
    }

    public func compileMessage() throws -> DerivableMessage {
        // verify instructions
        guard !instructions.isEmpty else {
            throw DerivableVersionedTransactionError.noInstructionProvided
        }
        guard let feePayer = feePayer else {
            throw DerivableVersionedTransactionError.feePayerNotFound
        }
        guard let recentBlockhash = recentBlockhash else {
            throw DerivableVersionedTransactionError.recentBlockhashNotFound
        }
...
//                        throw Error.invalidRequest(reason: "Transaction references a signature that is unnecessary")
                ...
                throw DerivableVersionedTransactionError.unknownSigner(signature.publicKey.base58EncodedString)
            }
        }

        // header
        var header = DerivableMessageHeader()
        ...
        try instructions.forEach { instruction in
            try instruction.accounts.forEach { keyIndex in
                if keyIndex < 0 { throw DerivableVersionedTransactionError.unknown }
            }
        }

        return DerivableMessage(
            header: header,
    ...
    private mutating func _serialize(serializedMessage: Data) -> Data {
        ...
        let signaturesData = signatures.reduce(Data()) { result, signature in
            ...
            if let signature = signature.signature {
                ...
                data.append(DerivableConstants.defaultSignature)
            }
            return data
        }
    ...
    public static func from(data: Data) throws -> DerivableTransaction {
        var data = data
        var signatures: [String] = []
        let signatureCount = data.decodeLength()

        for _ in stride(from: 0, through: signatureCount - 1, by: 1) {
            let signatureData = data.prefix(DerivableConstants.signatureLength)
            data = data.dropFirst(DerivableConstants.signatureLength)
            signatures.append(Base58.encode(signatureData))
        }

        print(data.base64EncodedString())
        return try populate(DerivableMessage.from(data: data), signatures)
    }

    static func populate(_ message: DerivableMessage, _ signatures: [String]) -> DerivableTransaction {
        ...
        signatures.enumerated().forEach { index, signature in
            let sigPubkeyPair = DerivableSignature(
                signature: signature == Base58
                    .encode(DerivableConstants.defaultSignature) ? nil : Data(Base58.decode(signature)),
...
public struct DerivableSignature: Encodable, Equatable {
...
private extension Array where Element == AccountMeta {
    func index(ofElementWithPublicKey publicKey: PublicKey) throws -> Int {
        ...
        else { throw DerivableVersionedTransactionError.unknown }
        return index
    }
}
