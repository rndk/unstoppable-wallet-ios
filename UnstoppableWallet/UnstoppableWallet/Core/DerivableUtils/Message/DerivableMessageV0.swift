...
public struct DerivableMessageV0: IMessage, Equatable {
    public var version: DerivableTransactionVersion { .v0 }

    public var header: DerivableMessageHeader
    public var staticAccountKeys: [PublicKey]
    public var recentBlockhash: DerivableBlockHash
    public var compiledInstructions: [DerivableMessageCompiledInstruction]
    public let addressTableLookups: [DerivableMessageAddressTableLookup]

    public init(
        header: DerivableMessageHeader,
        staticAccountKeys: [PublicKey],
        recentBlockhash: DerivableBlockHash,
        compiledInstructions: [DerivableMessageCompiledInstruction],
        addressTableLookups: [DerivableMessageAddressTableLookup]
    ) {
        self.header = header
    ...
    public func getAccountKeys(accountKeysFromLookups: DerivableAccountKeysFromLookups) -> DerivableMessageAccountKeys {
        .init(
    ...
    public func getAccountKeys(addressLookupTableAccounts: [DerivableAddressLookupTableAccount]) throws -> DerivableMessageAccountKeys {
    ...
    public func resolveAddressTableLookups(
        addressLookupTableAccounts: [DerivableAddressLookupTableAccount]
    ) throws -> DerivableAccountKeysFromLookups {
        var accountKeysFromLookups = DerivableAccountKeysFromLookups(readonly: [], writable: [])
        ...
        for tableLookup in addressTableLookups {
            ...
            guard let tableAccount = tableAccount else {
                throw DerivableVersionedMessageError
                    .other(
...
//        let instructionsLength = Data.encodeLength(compiledInstructions.count)
    ...
    public static func compile(
        ...
        var compiledKeys = DerivableCompiledKeys.compile(instructions: instructions, payer: payerKey)

        var addressTableLookups: [DerivableMessageAddressTableLookup] = []
        var accountKeysFromLookups: DerivableAccountKeysFromLookups = .init(readonly: [], writable: [])
    ...
    public static func deserialize(serializedMessage: Data) throws -> Self {
        ...
        let maskedPrefix: UInt8 = prefix & DerivableConstants.versionPrefixMask
        guard prefix != maskedPrefix else {
            throw DerivableVersionedMessageError.expectedVersionedMessageButReceivedLegacyMessage
        }

        let version = maskedPrefix
        guard version == 0 else {
            throw DerivableVersionedMessageError.invalidMessageVersion(expectedVersion: 0, receivedVersion: version)
        }

        let header = try DerivableMessageHeader(
        ...
        var compiledInstructions: [DerivableMessageCompiledInstruction] = []
