...
struct BlockchainSettingsView: View {
  ...
  @State private var derivableBlockchain: Blockchain?
  
  var body: some View {
    ScrollableThemeView {
      VStack(spacing: .margin32) {
        ListSection {
          ForEach(viewModel.derivableItems, id: \.blockchain.uid) { item in
            ClickableRow(action: {
              derivableBlockchain = item.blockchain
            }) {
              ItemView(
          ...
          .sheet(item: $derivableBlockchain) { blockchain in
            DerivableNetworkView(blockchain: blockchain)
          }
        }
