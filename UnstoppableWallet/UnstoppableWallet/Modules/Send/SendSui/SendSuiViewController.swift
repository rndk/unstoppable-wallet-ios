...
class SendSuiViewController: ThemeViewController {
  
  private let coinKitWrapper: SuiKitWrapper
  private let viewModel: SendSuiViewModel
  ...
  override func viewDidLoad() {
    ...
    iconImageView.setImage(
      withUrlString: viewModel.token.coin.imageUrl,
      placeholder: UIImage(named: viewModel.token.placeholderImageName)
    )
    ...
    buttonCell.onTap = { [weak self] in self?.didTapProceed() }
  ...
  private func openConfirm(sendData: SendSuiService.SendData) {
    guard let viewController = SendSuiConfirmationModule.viewController(coinKitWrapper: coinKitWrapper, sendData: sendData) else {
      return
    }
...
extension SendSuiViewController: SectionsDataSource {
