...
class SendSuiConfirmationViewModel {
  
  private let disposeBag = DisposeBag()
  
  private let service: SendSuiConfirmationService
  ...
  private let feesRelay = PublishRelay<[SuiFeeViewItem]>()
  
  init(
    service: SendSuiConfirmationService,
  ...
  private func sync(state: SendSuiConfirmationService.State) {
    switch state {
    case .ready(let fee):
      feesRelay.accept(feeItems(fee: fee))
    ...
    case .notReady(let errors):
      ...
      let cautions = errors.map { error in
        if let someError = error as? SendSuiConfirmationService.TransactionError {
          switch someError {
        ...
        } else if let someError = error as? SuiKit.SendError {
          switch someError {
          case .abnormalSend:
            return TitledCaution(
              title: "Error",
              text: "fee_settings.errors.unexpected_error".localized,
              type: .error
            )
          case .dryRunIsNil:
            return TitledCaution(
              title: "Error",
              text: "fee_settings.errors.dry_run".localized,
              type: .error
            )
  ...
  private func sync(sendState: SendSuiConfirmationService.SendState) {
    switch sendState {
    case .idle: ()
  ...
  private func feeItems(fee: BigUInt) -> [SuiFeeViewItem] {
    var viewItems = [SuiFeeViewItem]()
  ...
  private func items(
    dataState: SendSuiConfirmationService.DataState
  ) -> [SectionViewItem] {
