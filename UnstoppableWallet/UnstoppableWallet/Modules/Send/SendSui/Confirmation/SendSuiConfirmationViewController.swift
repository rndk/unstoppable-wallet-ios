...
class SendSuiConfirmationViewController: ThemeViewController {
  
  let disposeBag = DisposeBag()
  
  let transactionViewModel: SendSuiConfirmationViewModel
  ...
  private var sectionViewItems = [SendSuiConfirmationViewModel.SectionViewItem]()
  private var feeViewItems = [SendSuiConfirmationViewModel.SuiFeeViewItem]()
  private let cautionCell = TitledHighlightedDescriptionCell()
  private var isLoaded = false
  
  init(transactionViewModel: SendSuiConfirmationViewModel, feeViewModel: SendFeeViewModel) {
    self.transactionViewModel = transactionViewModel
    self.feeViewModel = feeViewModel
    
    feeCell = FeeCell(viewModel: feeViewModel, title: "send.fee".localized, showInfoIcon: false, isLast: false)
  ...
  private func handleFeeItems(items: [SendSuiConfirmationViewModel.SuiFeeViewItem]) {
  ...
  private func row(
    viewItem: SendSuiConfirmationViewModel.ViewItem,
    rowInfo: RowInfo
  ) -> RowProtocol {
    switch viewItem {
    case let .subhead(iconName, title, value):
      return CellComponent.actionTitleRow(
        tableView: tableView,
        rowInfo: rowInfo,
        iconName: iconName,
        iconDimmed: true,
    ...
    case let .amount(iconUrl, iconPlaceholderImageName, coinAmount, currencyAmount, type):
      return CellComponent.amountRow(
        tableView: tableView,
        rowInfo: rowInfo,
        iconUrl: iconUrl,
    ...
    case let .address(title, value, valueTitle, contactAddress):
      ...
      if let contactAddress {
        onAddToContact = { [weak self] in
          ContactBookModule.showAddition(
            contactAddress: contactAddress,
            parentViewController: self
          )
        }
      }
      return CellComponent.fromToRow(
        tableView: tableView,
        rowInfo: rowInfo,
        title: title,
        value: value,
  ...
  private func section(
    sectionViewItem: SendSuiConfirmationViewModel.SectionViewItem,
    index: Int
  ) -> SectionProtocol {
    return Section(
      id: "section_\(index)",
      ...
      rows: sectionViewItem.viewItems.enumerated().map { index, viewItem in
        row(
          ...
          rowInfo: RowInfo(
            index: index,
            isFirst: index == 0,
            isLast: index == sectionViewItem.viewItems.count - 1
          )
        )
      }
    )
  ...
  private func doubleAmountRow(
    tableView: SectionsTableView,
    rowInfo: RowInfo,
    item: SendSuiConfirmationViewModel.SuiFeeViewItem
  ) -> RowProtocol {
...
extension SendSuiConfirmationViewController: SectionsDataSource {
