...
class DerivableNetworkViewController: ThemeViewController {
  private let viewModel: DerivableNetworkViewModel
  private let disposeBag = DisposeBag()
  
  private let tableView = SectionsTableView(style: .grouped)
  private let iconImageView = UIImageView()
  
  private var defaultViewItems = [DerivableNetworkViewModel.ViewItem]()
  private var customViewItems = [DerivableNetworkViewModel.ViewItem]()
  private var isLoaded = false
  
  init(viewModel: DerivableNetworkViewModel) {
  ...
  override func viewDidLoad() {
    ...
    navigationItem.rightBarButtonItem = UIBarButtonItem(
      title: "button.done".localized, style: .plain, target: self, action: #selector(onTapDone)
    )
  ...
  private func openAddNew() {
    let module = AddDerivableSyncSourceModule.viewController(blockchainType: viewModel.blockchainType)
    present(module, animated: true)
  }
  
}

extension DerivableNetworkViewController: SectionsDataSource {
  ...
  private func row(
    id: String,
    viewItem: DerivableNetworkViewModel.ViewItem,
